...
if [[ $push_images ]]; then
  ...
  services=(basket.api catalog.api identity.api ordering.api payment.api coupon.api loyalty.api webmvc webspa webstatus)
...
charts=(coupon-api loyalty-api eshop-common basket-api catalog-api identity-api mobileshoppingagg ordering-api ordering-backgroundtasks ordering-signalrhub payment-api webmvc webshoppingagg webspa webstatus webhooks-api webhooks-web)
